<template>
  <b-modal v-model="ptkModalShow" title="Masukkan PTK" size="xl" ok-only ok-variant="secondary" ok-title="Tutup" scrollable>
    <b-row>
      <b-col cols="6">
        <b-row>
        ...
        <b-table bordered striped :items="terdaftar" :fields="fields" show-empty :busy="isBusyTerdaftar">
          ...
          <template v-slot:cell(actions)="row">
            <b-button variant="danger" size="sm" @click="keluarkan(row.item.ptk_id)">Keluarkan</b-button>
          </template>
        </b-table>
        <b-row class="mt-2">
          <b-col md="6">
      ...
      <b-col cols="6">
        ...
        <b-table bordered striped :items="kosong" :fields="fields" show-empty :busy="isBusyKosong">
          ...
          <template v-slot:cell(actions)="row">
            <b-button variant="success" size="sm" @click="masukkan(row.item.ptk_id)">Masukkan</b-button>
          </template>
        </b-table>
        <b-row class="mt-2">
          <b-col md="6">
...
<script>
import { BRow, BCol, BTable, BFormInput, BButton, BSpinner, BPagination } from 'bootstrap-vue'
import eventBus from '@core/utils/eventBus'
export default {
  components: {
    BRow, BCol, BTable, BFormInput, BButton, BSpinner, BPagination
  },
  data() {
    return {
      ptkModalShow: false,
  ...
  created() {
    eventBus.$on('open-modal-ptk-jam', this.handleEvent);
  },
  methods: {
    handleEvent(data){
      this.jam_id = data.id
    ...
    getKosong(){
      ...
      this.$http.get('/jam/kosong', {
        ...
        if(getData.errors){
          ...
          this.ptkModalShow = true
        }
      })
    },
    hideModal(){
      this.ptkModalShow = false
      this.resetForm()
    },
    resetForm(){
      this.terdaftar = []
