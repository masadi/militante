<template>
  <b-modal v-model="modalAdminUnit" title="Cetak Rekap Presensi" size="lg" @hidden="hideModal" @ok="handleOk" no-close-on-esc no-close-on-backdrop>
...
export default {
  ...
  created() {
    eventBus.$on('open-modal-add-admin', this.handleEvent);
  },
  methods: {
    handleEvent(data){
      this.data_sekolah = data
      this.form.periode_aktif = this.user.semester.nama
      this.modalAdminUnit = true
    },
    hideModal(){
      this.modalAdminUnit = false
      this.resetForm()
    },
    resetForm(){
      this.data_sekolah = []
    ...
    handleSubmit(){
      ...
      this.$http.post('/referensi/post-admin', this.form).then(response => {
