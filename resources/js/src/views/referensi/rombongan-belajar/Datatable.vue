    ...
    <b-overlay :show="loading" rounded opacity="0.6" size="lg" spinner-variant="warning">
      <b-table bordered striped :items="items" :fields="fields" :sort-by.sync="sortBy" :sort-desc.sync="sortDesc" show-empty :busy="isBusy">
        ...
        <template v-slot:cell(actions)="row">
          <b-dropdown id="dropdown-dropleft" dropleft text="Aksi" variant="primary" size="sm" bg-variant="dark">
            ...
            <template v-if="row.item.tingkat_pendidikan_id == 11 || row.item.tingkat_pendidikan_id == 12">
              <b-dropdown-item href="javascript:void(0)" @click="aksi(row.item, 'id-pkl')"><id-badge-icon /> Cetak ID PKL</b-dropdown-item>
            </template>
            <!--b-dropdown-item href="javascript:void(0)" @click="aksi(row.item, 'ujian')"><calendar-event-icon /> Jadwal Ujian</b-dropdown-item>
            <b-dropdown-item href="javascript:void(0)" @click="aksi(row.item, 'pts')"><calendar-event-icon /> Jadwal PTS</b-dropdown-item>
            <b-dropdown-item href="javascript:void(0)" @click="aksi(row.item, 'pas')"><calendar-event-icon /> Jadwal Pas</b-dropdown-item-->
          </b-dropdown>
        </template>
      </b-table>
    </b-overlay>
