      ...
      <b-table bordered striped :items="items" :fields="fields" :sort-by.sync="sortBy" :sort-desc.sync="sortDesc" show-empty :busy="isBusy">
        ...
        <template v-slot:cell(actions)="row">
          <b-dropdown id="dropdown-dropleft" dropleft text="Aksi" variant="primary" size="sm">
            ...
            <b-dropdown-item href="javascript:void(0)" @click="aksi(row.item, 'id-card')"><qrcode-icon /> Cetak ID Card</b-dropdown-item>
            <b-dropdown-item href="javascript:void(0)" @click="aksi(row.item, 'edit')"><pencil-icon /> Edit</b-dropdown-item>
            <b-dropdown-item href="javascript:void(0)" @click="aksi(row.item, 'reset')"><shield-lock-icon /> Reset Password</b-dropdown-item>
            <b-dropdown-item href="javascript:void(0)" @click="aksi(row.item, 'excel')"><font-awesome-icon :icon="['far', 'file-excel']" size="lg" /> Rekap Presensi Excel</b-dropdown-item>
            <b-dropdown-item href="javascript:void(0)" @click="aksi(row.item, 'pdf')"><font-awesome-icon :icon="['far', 'file-pdf']" size="lg" /> Rekap Presensi PDF</b-dropdown-item>
          </b-dropdown>
        </template>
      </b-table>
    </b-overlay>
  ...
  methods: {
    aksi(item, aksi){
      this.$emit('aksi', {
        aksi: aksi,
        item: item,
        data_sekolah: this.meta.data_sekolah,
      })
    },
