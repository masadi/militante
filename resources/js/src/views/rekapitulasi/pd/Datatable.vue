...
export default {
  ...
  methods: {
    onRowSelected(items) {
      var peserta_didik_id = [];
      items.forEach(function(value, key) {
        peserta_didik_id.push(value.peserta_didik_id)
      })
      this.$emit('selected', {
        item: (peserta_didik_id.length) ? encodeURIComponent(window.btoa(peserta_didik_id.join())) : null,
      })
    },
    aksi(item, aksi){
      this.$emit('aksi', {
