...
export default {
  ...
  created() {
    eventBus.$on('open-modal-cetak-rekap-pd', this.handleEvent);
  },
  updated: function () {
  ...
  methods: {
    handleEvent(data){
      if(data){
        if(data.peserta_didik_id){
          this.peserta_didik_id = data.peserta_didik_id
          ...
          this.$swal({
            icon: 'error',
            text: 'Silahkan pilih Peserta Didik terlebih dahulu',
          })  
        }
      } else {
        this.$swal({
    ...
    handleSubmit(){
      var link_open = `/cetak/${this.file}-rekap/${this.format}/${this.aksi}/${this.mulai_tanggal}/${this.sampai_tanggal}/${this.peserta_didik_id}`
