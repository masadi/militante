          ...
          <div class="card-body">
            <div class="row">
              <div class="col-lg-3 pull-right">
                <button type="button" class="btn btn-block btn-primary" data-toggle="modal" onclick="tambah_even()"> <i class="fa fa-fw fa-plus-circle"></i> Tambah {{$judulmodul}} </button>
                
              </div>
              <div class="col-lg-3 pull-right">
                <a href='{{url("relfamilia/download")}}' class="btn btn-block btn-success">Download XLS</a>
              </div>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-header">
        <h4 class="modal-title">Tambah Data Relasaun Familia</h4>
...
<script type="text/javascript">
  ...
  $('#btnSave').on('click',function(){
    ...
    if (!form.valid()) {
      ...
      if (save_method == 'add') {
        url = '{{url("relfamilia/save")}}';
        console.log(url);
      } else {
        url = "{{url('relfamilia/updateaction')}}";
      }
      $.ajax({
        url: url,
        type: 'POST',
  ...
  function delete_even(ideven) {
    if (confirm('Anda Yakin Menghapus data yang dipilih?')) {
      $.ajax({
        url: "{{url('relfamilia/delete')}}/" + ideven,
        success: function(data) {
