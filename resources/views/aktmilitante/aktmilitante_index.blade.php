        ...
        <div class="card">
          ...
          <div class="card-body">
            <div class="row">
              ...
              <div class="col-lg-3 pull-right">
                <a href='{{url("aktmilitante/download")}}' class="btn btn-block btn-success">Download XLS</a>
              </div>
...
<script type="text/javascript">
  ...
  $('#btnSave').on('click',function(){
    ...
    if (!form.valid()) {
      ...
      if (save_method == 'add') {
        url = '{{url("aktmilitante/save")}}';
        console.log(url);
      } else {
        url = "{{url('aktmilitante/updateaction')}}";
      }
      $.ajax({
        url: url,
        type: 'POST',
  ...
  function edit_even(id) {
    ...
    $.ajax({
      url: "{{url('aktmilitante/update')}}/" + id,
      type: "GET",
      dataType: "JSON",
      success: function(data) {
        console.log(data);
  ...
  function delete_even(ideven) {
    if (confirm('Anda Yakin Menghapus data yang dipilih?')) {
      $.ajax({
        url: "{{url('aktmilitante/delete')}}/" + ideven,
        success: function(data) {
