...
<table id="table" class="table">
  ...
  <?php
  $row = "";$no=1;
  foreach ($ambilkelurahan as $kelurahan) {
    $row .= "<tr><td>".$no."</td><td>" . $kelurahan->nama_suco . "</td>";
    for ($n = 0; $n < count($idparacalon); $n++) {


        $querysaya = DB::table('hitung')
            ->leftjoin('saksi','hitung.saksihitung','saksi.idsaksi')
            ->leftjoin('kelurahan','saksi.kelurahansaksi','kelurahan.idkelurahan')
            ->leftjoin('profil','saksi.saksiuntuk','profil.idcaleg')
            ->where('profil.idcaleg','=',$idparacalon[$n])
            ->where('kelurahan.idkelurahan','=',$kelurahan->idkelurahan)
            ->groupby('profil.idcaleg')
            ->groupby('kelurahan.idkelurahan')
            ->get();
