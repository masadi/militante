        ...
        <div class="card">
          ...
          <div class="card-body">
            <div class="row">
              ...
              <div class="col-lg-3 pull-right">
                <a href='{{url("profisaun/download")}}' class="btn btn-block btn-success">Download XLS</a>
              </div>
...
<script type="text/javascript">
  ...
  $('#btnSave').on('click',function(){
    ...
    if (!form.valid()) {
      ...
      if (save_method == 'add') {
        url = '{{url("profisaun/save")}}';
        console.log(url);
      } else {
        url = "{{url('profisaun/updateaction')}}";
      }
      $.ajax({
        url: url,
        type: 'POST',
  ...
  function delete_even(ideven) {
    if (confirm('Anda Yakin Menghapus data yang dipilih?')) {
      $.ajax({
        url: "{{url('profisaun/delete')}}/" + ideven,
        success: function(data) {
